...
var base62 = require('base-x')('ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789')
...
function clone(giturl, repositoryPath, callback) {
    exec('git clone ' + giturl + ' ' + repositoryPath, {
        timeout: 30000
    }, function (error) {
        if (error !== null) {
            callback(error);
...
function getCommitObj(line) {
    ...
    try {
        ...
        console.error("ERROR: Function getCommitObj:" + error);
        console.error(error);
    }
    return commitObj;
}
...
function updateProvObj(provObject, urlprefix, commitObj) {
    ...
    provObject.ends[urlprefix + ":" + commitObj.id + "_end"] = {
        "prov:activity": urlprefix + ":" + commitObj.id,
        "prov:time": commitObj.commiter_date
    };
...
function getLogCmds(provObject, fileList, urlprefix, options) {
    ...
    var logCmds = files.map(function (file) {
        ...
        var currentEntity = base62.encode(buffer.Buffer.from(file));
        //console.log(currentEntity);
        var label = file;
        if (label.startsWith('"')) {
            label = "";
        }
        provObject.entities[urlprefix + ":file-" + currentEntity] = {
            "prov:label": label
        };
...
function convertRepositoryToProv(repositoryPath, serialization, requestUrl, options, callback) {
    ...
    exec('git --no-pager log --pretty=format: --name-only --diff-filter=A', {
        cwd: repositoryPath
        ...
        function executeLogCmd(logcmd) {
            exec(logcmd, {
                ...
                lines.forEach(function (line) {
                    var data = line.split(";");
                    if (data.length == 1) {
                        console.error("WARNING: Mysterious git log output: " + data);
                    } else {
                        line = line.replace(/['"]/g, '');
